...
dataset = pd.read_csv("Data_80avg.csv")
X = dataset.iloc[:,1:8].values #import all columns except the last one
y = dataset.iloc[:,0].values #import the last column
...
#plt.scatter(X_train[:,0], y_pred_train, color = 'blue')
#plt.title('Regression output')
plt.title('Scatter graph of electricity price vs residual load')
plt.xlabel('Normalised Residual Demand')
plt.ylabel('Normalised Electricity Price')
plt.show()
